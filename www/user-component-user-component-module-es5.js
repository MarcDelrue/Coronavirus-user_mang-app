(window["webpackJsonp"] = window["webpackJsonp"] || []).push([["user-component-user-component-module"], {
  ...
  function srcAppUserComponentUserComponentPageTs(module, __webpack_exports__, __webpack_require__) {
    ...
    var firebase_storage__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(
    /*! firebase/storage */
    "./node_modules/firebase/storage/dist/index.esm.js");

    let UserComponentPage = class UserComponentPage {
      constructor(route, navCtrl, fireStore, alert) {
        this.route = route;
      ...
      saveUpdates() {
        if (this.user.id) this.fireStore.updateUser(this.user).then(data => console.log(data));
      }

      createNewUser() {
        if (this.user.name) this.fireStore.createUser(this.user.name).then(id => {
          console.log(id);
          this.user.photos = [];
          this.user.id = id;
        });
      ...
      removePhoto(photo) {
        ...
        for (let x of this.user.photos) {
          if (photo.includes(x)) this.user.photos.splice(x, 1);
        }
